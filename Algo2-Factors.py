...
def make_pipeline():
    ...
    """
    sentiment_30day = SimpleMovingAverage(inputs=[sentiment.sentiment_signal], window_length=10)
    growthSc = Fundamentals.growth_score.latest
    
    senti = (sentiment_30day)*growthSc
    
    current_cash = Fundamentals.cash_and_cash_equivalents.latest
    current_assets = Fundamentals.current_assets.latest
    current_debt = Fundamentals.current_debt_and_capital_lease_obligation.latest
    invested_capital = Fundamentals.invested_capital.latest
     
    cash = (current_cash-current_debt)
    inv_as = current_assets/invested_capital
   
    universe = QTradableStocksUS()

    # Here we combine our winsorized factors, z-scoring them to equalize their influence
    combined_factor = (
        senti.zscore() + cash.zscore() + inv_as.zscore()
    )
