...
def make_pipeline():
    ...
    freeCash = Fundamentals.free_cash_flow.latest
    workCapital = Fundamentals.working_capital.latest
    totAssets = Fundamentals.total_assets.latest
    ebitda = Fundamentals.ebitda.latest
    
    money = (workCapital/totAssets) * (ebitda/freeCash)
    
    quality = Fundamentals.roe.latest
    growthSc = Fundamentals.growth_score.latest
    ...
    money_w = money.winsorize(min_percentile=0.05, max_percentile=0.95)
    quality_w = quality.winsorize(min_percentile=0.05, max_percentile=0.95)
    growthSc_w = growthSc.winsorize(min_percentile=0.05, max_percentile=0.95)

    # Here we combine our winsorized factors, z-scoring them to equalize their influence
    combined_factor = (
        (quality_w.zscore()+growthSc_w.zscore())+money_w.zscore()
    )
